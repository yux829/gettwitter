...
def main():
    ...
    data=[]
    header_list = ['时间', '内容']
    layout = [[sg.T('*推特用户名:',size=(10,1)),sg.InputText (key='key_query_name',default_text="realsatoshinet",size=(25,1)),sg.T('时间段从:',size=(8,1)),sg.InputText (key='key_since',default_text=since,size=(15,1)),sg.T('至:',size=(3,1),justification='right'),sg.InputText (key='key_util',default_text=until,size=(15,1)),sg.Button('查询')],[sg.T('处理方式:',size=(10,1),justification='right'),sg.Radio('新获取', "query_type",key='key_query_type_new', default=True, size=(5,1)), sg.Radio('只看不获取', "query_type",key='key_query_type_view'),sg.Radio('深度采集(耗时长)', "query_type",key='key_query_type_all')],[sg.T('代理服务器:',size=(10,1)),sg.InputText(key='key_proxy',size=(70,2),default_text='127.0.0.1:11000')],[sg.InputText(default_text='备注：推特反爬虫技术过于厉害，查询结果可能不全，望包涵!',font=("微软雅黑", 10),justification='left',readonly = True,size=(110,1))],[sg.Multiline(key='key_output',size=(100, 20), font=('微软雅黑 12'))]]
    ...
    while True:
        ...
        if event == '查询':
            ...
            print(queryName+'-'+since+'-'+until+'-'+str(key_query_type_new)+'-'+str(queryTypeIsView)+'-'+str(queryTypeIsAll))
            if queryName :
                datas = gettwitter.getAndSaveAndShow(queryName,since,until,queryTypeIsView,queryTypeIsAll,proxy)
                dataBuffer =''
                if datas:
