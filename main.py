    ...
    while True:
        ...
        if event == '查询':
            ...
            if until:
                ...
                until =(datetime.datetime.now()).strftime("%Y-%m-%d")
            until =gettwitter.getNextday(until,1)
            ...
            if query_model=='用户名' :
                ...
                keyName=queryName
            pdfFileName=(queryName+''+since+'to'+until).replace('-','')+'.pdf'
            if queryName :
